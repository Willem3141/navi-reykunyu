...
import { _, setNewLanguage } from './ui-translations';
...
class Reykunyu {
	...
	imageSection(word: WordData, image: string): JQuery {
		...
		$('<img/>').attr('src', '/ayrel/' + image)
			.appendTo($section);
		$('<div/>').addClass('credit')
			.html(this.lemmaForm(word) + ' ' + _('image-drawn-by') + ' Eana Unil')
			.appendTo($section);
		return $section;
	}
	...
	affixesSection(affixes: AffixData[]) {
		...
		for (let a of affixes) {
			...
			if (a.hasOwnProperty('combinedFrom')) {
				let $affixSpan = $('<span/>')
					.html(this.lemmaForm(<WordData>{ 'word': { 'FN': <string>affix, 'combined': <string>affix, 'RN': <string>affix }, 'type': 'aff:in' }));
				this.addLemmaClass($affixSpan, 'aff:in');
				$('<td/>')
					.append($affixSpan)
				...
				for (const c of (<CombinedAffixData>a)['combinedFrom']) {
					...
					let $affixLink = $('<a/>')
						.addClass('word-link')
						.html(this.lemmaForm(c['affix']))
						.attr('href', '/?q=' + c['affix']["word_raw"][this.getDialect()]);
					this.addLemmaClass($affixLink, c['affix']['type']);
				...
				let $affixLink = $('<a/>')
					.addClass('word-link')
					.html(this.lemmaForm(<WordData>affix))
					//.addClass(a['type'])
					.attr('href', '/?q=' + (<WordData>affix)["word_raw"][this.getDialect()]);
				this.addLemmaClass($affixLink, (<WordData>affix)['type']);
				$('<td/>').append($affixLink)
	...
	createWordLink(link: LinkStringPiece): JQuery {
		if (typeof link === "string") {
			...
			let $word = $('<span/>')
				.addClass('navi')
				.html(this.lemmaForm(link));
			this.addLemmaClass($word, link["type"]);
			$link.append($word);
	...
	lemmaForm(word: WordData): string {
		let type = word['type'];
		let lemma = word['word'][this.getDialect()];
	...
	addLemmaClass($element: JQuery, type: string) {
	...
	createResultBlock(i: number, r: WordData) {
		...
		this.addLemmaClass($lemma, r['type']);
		$lemma.html(this.lemmaForm(r));
